...
//  - 支持 CentOS 7/9 + Node 16/18/20/22
// ====================================================================

// 兼容旧版 Node.js（Node 18+ 内置 fetch，无需 polyfill）
...
const puppeteerLogin = require('./puppeteer-login');

const PLATFORM_QR_URLS = {
  netease: 'https://music.163.com/',
  qq: 'https://y.qq.com/',
  kugou: 'https://www.kugou.com/',
  kuwo: 'https://www.kuwo.cn/',
  qishui: 'https://www.qishui.com/',
...
let kuwoCookie = '';
try { if (fs.existsSync(KUWO_COOKIE_FILE)) kuwoCookie = fs.readFileSync(KUWO_COOKIE_FILE, 'utf8').trim(); }
catch (e) { kuwoCookie = ''; }
function saveKuwoCookie(c) {
  kuwoCookie = normalizeCookieHeader(c) || rawCookieFallback(c);
...
try {
  if (fs.existsSync(SPOTIFY_TOKEN_FILE)) {
    const raw = fs.readFileSync(SPOTIFY_TOKEN_FILE, 'utf8').trim();
    spotifyToken = JSON.parse(raw);
  }
} catch (e) { spotifyToken = null; }
function saveSpotifyToken(t) {
  spotifyToken = t || null;
  try { fs.writeFileSync(SPOTIFY_TOKEN_FILE, JSON.stringify(spotifyToken || {})); } catch (e) {}
}
...
// ====================================================================
//  🧑‍🤝‍🧑 多用户独立 Cookie 系统（v3）
//  每个浏览器用户有独立的 userToken，各自维护自己的登录 Cookie
//  不再共用全局账号，默认未登录状态
// ====================================================================
const userSessions = new Map(); // userToken → { netease, qq, kugou, kuwo, qishui, spotify, createdAt }

function getUserToken(req) {
  // 优先从 header 获取，其次从 query param
  let token = (req.headers['x-user-token'] || '').trim();
  if (!token) {
    try {
      const urlObj = new URL(req.url, 'http://localhost');
      token = (urlObj.searchParams.get('userToken') || '').trim();
    } catch (e) {}
  }
  return token || null;
}

function getOrCreateUserSession(token) {
...
function getUserCookie(provider, token) {
  const sess = getOrCreateUserSession(token);
  if (!sess) {
    // 无 token 时回退到全局 cookie（兼容旧版前端）
    if (provider === 'netease') return userCookie;
    if (provider === 'qq') return qqCookie;
    if (provider === 'kugou') return kugouCookie;
    if (provider === 'kuwo') return kuwoCookie;
    if (provider === 'qishui') return qishuiCookie;
    return '';
  }
  return sess[provider] || '';
}

function setUserCookie(provider, token, cookieValue) {
  const sess = getOrCreateUserSession(token);
  if (!sess) return;
  if (provider === 'spotify') {
    sess.spotify = cookieValue;
  } else {
    sess[provider] = normalizeCookieHeader(cookieValue) || rawCookieFallback(cookieValue) || '';
  }
}
...
setInterval(() => {
  ...
  for (const [token, sess] of userSessions.entries()) {
    if (now - sess.lastActive > MAX_AGE) {
      userSessions.delete(token);
    }
  }
}, 60 * 60 * 1000); // 每小时清理一次
...
async function kugouRequest(url, opts) {
  opts = opts || {};
  const headers = {
    'User-Agent': KUGOU_UA,
  ...
  if (opts.cookie && kugouCookie) headers['Cookie'] = kugouCookie;
...
function mapKugouSong(s) {
  ...
  return {
    ...
    source: 'kugou',
    type: 'song',
    id: s.FileHash || s.hash || s.HASH || s.SongHash || '',
    name: s.SongName || s.songname || s.filename || '',
    artist: s.SingerName || s.singername || s.authors || '',
    artists: [],
    artistId: '',
    album: s.AlbumName || s.albumname || '',
    cover: s.AlbumImage || s.imgUrl || s.Image || '',
    duration: (s.Duration || s.duration || s.Durtion || 0) * 1000,
    hash: s.FileHash || s.hash || s.HASH || '',
...
async function handleKugouSearch(keywords, limit) {
  ...
  const url = `https://complexsearch.kugou.com/v2/search/song?keyword=${encodeURIComponent(kw)}&page=1&pagesize=${limit}&bitrate=0&isfuzzy=0&inputtype=0&platform=WebFilter&userid=0&clientver=2000&iscorrection=1&privilege_filter=0&filter=10&area_code=1`;
  const data = await kugouRequest(url);
  const list = data && data.data && data.data.lists ? data.data.lists : [];
  return list.map(mapKugouSong).filter(s => s.name);
}

async function handleKugouSongUrl(hash, albumId) {
  const songHash = String(hash || '').trim();
  if (!songHash) return { provider: 'kugou', url: '', playable: false, error: 'MISSING_HASH' };
  const url = `https://wwwapi.kugou.com/yy/index.php?r=play/getdata&hash=${encodeURIComponent(songHash)}&dfid=0&mid=0&platid=4&_=${Date.now()}`;
  try {
    const data = await kugouRequest(url);
    const playUrl = data && data.data && data.data.play_url ? data.data.play_url : '';
    if (playUrl) {
      return { provider: 'kugou', url: playUrl, playable: true, level: 'standard', quality: '标准音质' };
    }
    return { provider: 'kugou', url: '', playable: false, error: 'URL_UNAVAILABLE', message: '酷狗音乐未返回播放地址' };
  } catch (err) {
    return { provider: 'kugou', url: '', playable: false, error: err.message };
  }
}

async function handleKugouLyric(hash) {
  const songHash = String(hash || '').trim();
  if (!songHash) return { provider: 'kugou', lyric: '' };
  const url = `https://wwwapi.kugou.com/yy/index.php?r=play/getdata&hash=${encodeURIComponent(songHash)}&dfid=0&mid=0&platid=4&_=${Date.now()}`;
  try {
    const data = await kugouRequest(url);
    const lyrics = data && data.data && data.data.lyrics ? data.data.lyrics : '';
    return { provider: 'kugou', lyric: lyrics };
  } catch (err) {
    return { provider: 'kugou', lyric: '' };
  }
}
...
//  新增平台: 酷我音乐 (Kuwo)
// ====================================================================
const KUWO_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';

async function kuwoRequest(url, opts) {
  opts = opts || {};
  const headers = {
    'User-Agent': KUWO_UA,
  ...
  if (opts.cookie && kuwoCookie) headers['Cookie'] = kuwoCookie;
  const resp = await fetch(url, { headers, signal: opts.signal });
  if (!resp.ok) throw new Error('HTTP ' + resp.status);
  return resp.json();
}

function mapKuwoSong(s) {
  s = s || {};
  const artists = [];
  if (s.artist) artists.push({ name: s.artist });
  else if (s.ARTIST) artists.push({ name: s.ARTIST });
  return {
    provider: 'kuwo',
    source: 'kuwo',
    type: 'song',
    id: s.rid || s.RID || s.musicrid || s.MUSICRID || '',
    name: s.name || s.NAME || s.songName || '',
    artist: s.artist || s.ARTIST || s.songer || '',
    artists,
    artistId: s.artistid || s.ARTISTID || '',
    album: s.album || s.ALBUM || '',
    cover: s.pic || s.pic120 || s.pic160 || s.pic240 || s.pic480 || s.pic1080 || '',
    duration: (s.duration || s.DURATION || 0) * 1000,
    rid: s.rid || s.RID || s.musicrid || s.MUSICRID || '',
  };
}
...
async function handleKuwoSearch(keywords, limit) {
  const kw = String(keywords || '').trim();
  if (!kw) return [];
  console.log('[KuwoSearch]', kw, 'limit:', limit);
  const url = `https://kuwo.cn/api/www/search/searchMusicBykeyWord?key=${encodeURIComponent(kw)}&pn=1&rn=${limit}&httpsStatus=1&reqId=0`;
  try {
    const data = await kuwoRequest(url);
    const list = data && data.data && data.data.list ? data.data.list : [];
    return list.map(mapKuwoSong).filter(s => s.name);
  } catch (err) {
...
async function handleKuwoSongUrl(rid) {
  const musicRid = String(rid || '').trim();
  if (!musicRid) return { provider: 'kuwo', url: '', playable: false, error: 'MISSING_RID' };
  const url = `https://kuwo.cn/url?rid=${encodeURIComponent(musicRid)}&type=convert_url3&br=128kmp3`;
  try {
    const data = await kuwoRequest(url);
    const playUrl = data && data.url ? data.url : '';
    if (playUrl) {
      return { provider: 'kuwo', url: playUrl, playable: true, level: 'standard', quality: '128k MP3' };
    }
    return { provider: 'kuwo', url: '', playable: false, error: 'URL_UNAVAILABLE', message: '酷我音乐未返回播放地址' };
  } catch (err) {
    return { provider: 'kuwo', url: '', playable: false, error: err.message };
  }
}

async function handleKuwoLyric(rid) {
  const musicRid = String(rid || '').trim();
  if (!musicRid) return { provider: 'kuwo', lyric: '' };
  const url = `https://kuwo.cn/openapi/v1/www/lyric/getLyricByMusicId?musicId=${encodeURIComponent(musicRid)}&httpsStatus=1&reqId=0`;
  try {
    const data = await kuwoRequest(url);
    const lyrics = data && data.data && data.data.lrclist ? data.data.lrclist.map(item => `[${item.time}]${item.lineLyric}`).join('\n') : '';
    return { provider: 'kuwo', lyric: lyrics };
  } catch (err) {
    return { provider: 'kuwo', lyric: '' };
  }
}

// ====================================================================
//  新增平台: 汽水音乐 (Qishui / Soda Music)
// ====================================================================
const QISHUI_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';

async function qishuiRequest(url, opts) {
  opts = opts || {};
  const headers = {
...
function mapQishuiSong(s) {
  s = s || {};
  return {
    provider: 'qishui',
    source: 'qishui',
    type: 'song',
    id: s.id || s.music_id || s.song_id || '',
    name: s.title || s.name || s.song_name || '',
    artist: s.author || s.artist || s.singer || '',
    artists: [],
    artistId: s.author_id || s.artist_id || '',
    album: s.album || s.album_name || '',
    cover: s.cover || s.cover_url || s.poster || '',
...
async function handleQishuiSearch(keywords, limit) {
  const kw = String(keywords || '').trim();
  if (!kw) return [];
  console.log('[QishuiSearch]', kw, 'limit:', limit);
  // 汽水音乐使用抖音搜索接口
  const url = `https://open.douyin.com/api/search/suggest/?query=${encodeURIComponent(kw)}&count=${limit}&type=2`;
  try {
    const data = await qishuiRequest(url);
    const list = data && data.data && data.data.suggest_list ? data.data.suggest_list : [];
    return list.map(mapQishuiSong).filter(s => s.name);
  } catch (err) {
...
async function handleQishuiSongUrl(id) {
  const songId = String(id || '').trim();
  if (!songId) return { provider: 'qishui', url: '', playable: false, error: 'MISSING_ID' };
  // 汽水音乐播放地址需要通过第三方解析
  return { provider: 'qishui', url: '', playable: false, error: 'NOT_IMPLEMENTED', message: '汽水音乐播放地址需要登录授权' };
}

async function handleQishuiLyric(id) {
  const songId = String(id || '').trim();
  if (!songId) return { provider: 'qishui', lyric: '' };
  return { provider: 'qishui', lyric: '' };
}
...
async function handleKugouUserPlaylists() {
  if (!kugouCookie) return { loggedIn: false, provider: 'kugou', playlists: [] };
  try {
    const u = new URL('http://www.kugou.com/yy/index.php');
    u.searchParams.set('r', 'user/getUserInfo');
    const headers = { Cookie: kugouCookie, 'User-Agent': UA, Referer: 'http://www.kugou.com/' };
    const text = await requestText(u.toString(), { headers });
    const info = parseJSONText(text);
    const userId = (info && info.userid) || (info && info.user_id) || '';
    if (!userId) return { loggedIn: true, provider: 'kugou', playlists: [], userId: '', message: '无法获取用户ID' };

    // 获取用户创建的歌单
    const u2 = new URL('http://mobileservice.kugou.com/v2/songlist/my');
    u2.searchParams.set('userid', userId);
    u2.searchParams.set('page', '1');
    u2.searchParams.set('pagesize', '100');
    const headers2 = { ...headers, 'kg-token': '' };
    const text2 = await requestText(u2.toString(), { headers: headers2 });
    const data = parseJSONText(text2);
    const rawLists = ((data && data.lists) || (data && data.data && data.data.lists) || []);
    const playlists = rawLists.map(pl => ({
      id: String(pl.listid || pl.id || pl.playlistID || ''),
      name: pl.name || pl.list_name || pl.title || '未命名歌单',
      cover: pl.img || pl.pic || pl.cover || '',
      trackCount: pl.songcount || pl.song_count || pl.total || 0,
      playCount: 0,
      creator: pl.username || pl.nickname || pl.creator || '',
      subscribed: false,
      specialType: 0,
    })).filter(pl => pl.id && pl.name);
    return { loggedIn: true, provider: 'kugou', userId: String(userId), playlists };
  } catch (e) {
    console.error('[KugouPlaylists]', e.message);
    return { loggedIn: !!kugouCookie, provider: 'kugou', playlists: [], error: e.message };
  }
}
...
//  酷我用户歌单
// ====================================================================
async function handleKuwoUserPlaylists() {
  if (!kuwoCookie) return { loggedIn: false, provider: 'kuwo', playlists: [] };
  try {
    const tokenMatch = kuwoCookie.match(/kw_token=([^;]+)/);
    const token = tokenMatch ? tokenMatch[1] : '';
    if (!token) return { loggedIn: true, provider: 'kuwo', playlists: [], message: '缺少登录凭证' };
    ...
    // 获取用户信息
    const profileText = await requestText('http://www.kuwo.cn/api/www/user/userInfo', { headers });
    const profileData = parseJSONText(profileText);
    const userInfo = (profileData && profileData.data) || profileData || {};
    const uid = userInfo.uid || userInfo.id || userInfo.userId || '';

    // 获取收藏的歌单
    const listText = await requestText('http://www.kuwo.cn/api/www/playlist/subscribeList?pid=1&pn=1&rn=100&order=create_time', { headers });
    const listData = parseJSONText(listText);
    const rawLists = ((listData && listData.data) || (listData && listData.list) || []);
    const playlists = Array.isArray(rawLists) ? rawLists.map(pl => ({
      id: String(pl.id || pl.pid || pl.playlistId || ''),
      name: pl.name || pl.title || '未命名歌单',
      cover: pl.img || pl.pic || pl.cover || '',
      trackCount: pl.trackCount || pl.total || pl.songnum || 0,
      playCount: pl.playCount || pl.listenCount || 0,
      creator: pl.username || pl.creator || pl.nick || '',
      subscribed: true,
      specialType: 0,
    })).filter(pl => pl.id && pl.name) : [];
    return { loggedIn: true, provider: 'kuwo', userId: String(uid), playlists };
  } catch (e) {
    console.error('[KuwoPlaylists]', e.message);
    return { loggedIn: !!kuwoCookie, provider: 'kuwo', playlists: [], error: e.message };
  }
}
...
async function handleQishuiUserPlaylists() {
  if (!qishuiCookie) return { loggedIn: false, provider: 'qishui', playlists: [] };
  try {
    const headers = { Cookie: qishuiCookie, 'User-Agent': UA };
    // 汽水音乐 API - 获取用户信息
    const profileText = await requestText('https://qishui.douyin.com/api/v1/user/info', { headers });
    const profile = parseJSONText(profileText);
    const uid = (profile && (profile.user_id || profile.uid || profile.id)) || '';

    // 构建歌单列表（汽水的"我喜欢"）
    const playlists = [];
    // 尝试获取收藏歌曲作为"我喜欢"
    try {
      const favText = await requestText('https://qishui.douyin.com/api/v1/user/favorite/songs?limit=100&offset=0', { headers });
      const favData = parseJSONText(favText);
      const favSongs = ((favData && favData.songs) || (favData && favData.data) || []);
      if (Array.isArray(favSongs) && favSongs.length > 0) {
        playlists.push({ id: 'favorite', name: '我喜欢的音乐', cover: '', trackCount: favSongs.length, playCount: 0, creator: '', subscribed: false, specialType: 5 });
      }
    } catch (_) {}
    // 尝试获取用户的歌单
    try {
      const listText = await requestText('https://qishui.douyin.com/api/v1/user/playlists?limit=50&offset=0', { headers });
      const listData = parseJSONText(listText);
      const userLists = ((listData && listData.playlists) || (listData && listData.data) || []);
      if (Array.isArray(userLists)) {
        userLists.forEach(pl => {
          playlists.push({
            id: String(pl.id || ''), name: pl.name || pl.title || '未命名',
            cover: pl.cover || pl.pic || pl.image_url || '',
            trackCount: pl.track_count || pl.song_count || pl.total || 0,
            playCount: 0, creator: '', subscribed: false, specialType: 0,
          });
        });
      }
    } catch (_) {}
    return { loggedIn: true, provider: 'qishui', userId: String(uid), playlists };
  } catch (e) {
    console.error('[QishuiPlaylists]', e.message);
    return { loggedIn: !!qishuiCookie, provider: 'qishui', playlists: [], error: e.message };
  }
}
...
async function spotifyRequest(url, opts) {
  opts = opts || {};
...
function mapSpotifySong(s) {
  s = s || {};
  const artists = (s.artists || []).map(a => ({ id: a.id, name: a.name || '' })).filter(a => a.name);
  const album = s.album || {};
  return {
    provider: 'spotify',
    source: 'spotify',
...
async function handleSpotifySearch(keywords, limit) {
  ...
  try {
    const token = await getSpotifyToken();
    if (!token) return [];
    const url = `https://api.spotify.com/v1/search?q=${encodeURIComponent(kw)}&type=track&limit=${limit}&market=CN`;
    const data = await spotifyRequest(url);
    const list = data && data.tracks && data.tracks.items ? data.tracks.items : [];
    return list.map(mapSpotifySong).filter(s => s.name);
  } catch (err) {
...
async function handleSpotifySongUrl(id) {
  const songId = String(id || '').trim();
  if (!songId) return { provider: 'spotify', url: '', playable: false, error: 'MISSING_ID' };
  // Spotify requires user-level OAuth for playback; preview URL is available for some tracks
  try {
    const token = await getSpotifyToken();
    if (!token) return { provider: 'spotify', url: '', playable: false, error: 'SPOTIFY_NOT_CONFIGURED' };
    const url = `https://api.spotify.com/v1/tracks/${encodeURIComponent(songId)}?market=CN`;
    const data = await spotifyRequest(url);
    const previewUrl = data && data.preview_url ? data.preview_url : '';
    if (previewUrl) {
      return { provider: 'spotify', url: previewUrl, playable: true, level: 'preview', quality: 'Preview', preview: true };
    }
    return { provider: 'spotify', url: '', playable: false, error: 'PREVIEW_UNAVAILABLE', message: 'Spotify 未提供预览地址' };
  } catch (err) {
    return { provider: 'spotify', url: '', playable: false, error: err.message };
  }
}

async function handleSpotifyLyric(id) {
  const songId = String(id || '').trim();
  if (!songId) return { provider: 'spotify', lyric: '' };
  try {
    const token = await getSpotifyToken();
    if (!token) return { provider: 'spotify', lyric: '' };
    const url = `https://spclient.wg.spotify.com/color-lyrics/v2/track/${encodeURIComponent(songId)}?format=json&vocalRemoval=false&market=from_token`;
    const resp = await fetch(url, { headers: { 'Authorization': 'Bearer ' + token, 'App-Platform': 'WebPlayer' } });
    if (!resp.ok) return { provider: 'spotify', lyric: '' };
    const data = await resp.json();
    const lines = data && data.lyrics && data.lyrics.lines ? data.lyrics.lines : [];
    const lrc = lines.map(line => `[${formatMsToLrcTime(line.startTimeMs || 0)}]${line.words || ''}`).join('\n');
    return { provider: 'spotify', lyric: lrc };
  } catch (err) {
    return { provider: 'spotify', lyric: '' };
  }
}
...
// ====================================================================
//  🎯 网易云 API 二维码登录（模块级，无需 Puppeteer，稳定可靠）
// ====================================================================
const activePuppeteerSessions = new Map(); // sessionId → session (兼容 API + Puppeteer 模式)

function startNeteaseApiPolling(sessionId, qrKey) {
  const maxDuration = 5 * 60 * 1000; // 5 分钟超时
  const interval = 2500;             // 2.5 秒检查一次

  const pollTimer = setInterval(async () => {
    const session = activePuppeteerSessions.get(sessionId);
    if (!session || session.status === 'logged_in' || session.status === 'error' || session.status === 'expired') {
      clearInterval(pollTimer);
      return;
    }

    if (Date.now() - session.createdAt > maxDuration) {
      session.status = 'expired';
      session.error = '二维码已过期，请刷新重试';
      clearInterval(pollTimer);
      activePuppeteerSessions.delete(sessionId);
      return;
    }

    try {
      const r = await login_qr_check({ key: qrKey, timestamp: Date.now() });
      const body = r.body || {};
      ...
      if (code === 800) {
        ...
        session.status = 'waiting'; // 等待扫码
      } else if (code === 802) {
        session.status = 'scanning'; // 已扫码，待确认
      } else if (code === 803) {
        let cookie = null;
        try {
          const retryR = await login_qr_check({ key: qrKey, timestamp: Date.now() });
          cookie = readCookieFromResponse(retryR) || readCookieFromResponse(r);
        } catch (e) {
          cookie = readCookieFromResponse(r);
        }
        session.status = 'logged_in';
        session.cookies = cookie;
        clearInterval(pollTimer);
        console.log('[NetEaseAPI] ✅ 二维码登录成功! sessionId:', sessionId.substr(0, 12) + '...');
        setTimeout(() => { activePuppeteerSessions.delete(sessionId); }, 60000);
      }
    } catch (e) {
      console.warn('[NeteaseAPI] poll error:', e.message);
    }
...
function readCookieFromResponse(r) {
  if (!r) return null;
  const dataCookie = r.body && r.body.data && r.body.data.cookie;
  if (dataCookie) return dataCookie;
  const bodyCookie = r.body && r.body.cookie;
...
const server = http.createServer(async (req, res) => {
  ...
  // 🔍 调试日志：打印每个请求的路径（部署完成后可删除此行）
  console.log('[DEBUG] req:', req.method, pn);

  // 🧑‍🤝‍🧑 多用户 Cookie 系统：
  // - 有 userToken 且有会话 → 用该用户的 cookie
  // - 没有 userToken / 无会话 → 清空所有 cookie（访客状态，不共享管理员账号）
  const _currentUserToken = getUserToken(req);
  let _restoreGlobalCookie = { userCookie, qqCookie, kugouCookie, kuwoCookie, qishuiCookie, spotifyToken };

  if (_currentUserToken) {
    const _sess = userSessions.get(_currentUserToken);
    if (_sess) {
      // 用当前用户的 cookie 替换全局变量（API 内部读取这些变量）
      userCookie = _sess.netease || '';
      qqCookie = _sess.qq || '';
      kugouCookie = _sess.kugou || '';
      kuwoCookie = _sess.kuwo || '';
      qishuiCookie = _sess.qishui || '';
      spotifyToken = _sess.spotify || null;
    } else {
      // token 无效 → 访客状态
      userCookie = ''; qqCookie = ''; kugouCookie = ''; kuwoCookie = ''; qishuiCookie = ''; spotifyToken = null;
    }
  } else {
  ...
  // ---------- 多用户会话管理 ----------
  if (pn === '/api/user/session') {
    // GET: 获取或创建用户会话
    // DELETE: 注销用户会话
    if (req.method === 'DELETE') {
      const token = getUserToken(req);
      if (token) clearUserSession(token);
    ...
    if (!token) {
      // 生成新 token
      token = 'u_' + Date.now().toString(36) + '_' + Math.random().toString(36).substr(2, 12);
    }
    const sess = getOrCreateUserSession(token);
    const platformLogins = {};
    for (const p of ['netease', 'qq', 'kugou', 'kuwo', 'qishui']) {
      platformLogins[p] = !!(sess && sess[p]);
    }
    platformLogins['spotify'] = !!(sess && sess.spotify);
    sendJSON(res, {
      token,
      platforms: platformLogins,
      createdAt: sess ? sess.createdAt : null,
  ...
  // ---------- Puppeteer 诊断 ----------
  console.log('[DEBUG] checking diag route, pn=', JSON.stringify(pn));
  if (pn === '/api/puppeteer/diag') {
    console.log('[DEBUG] ✅ diag route MATCHED!');
    sendJSON(res, {
      available: puppeteerLogin.isAvailable ? puppeteerLogin.isAvailable() : false,
      diag: puppeteerLogin.getDiag ? puppeteerLogin.getDiag() : null,
      cwd: process.cwd(),
      nodeVersion: process.version,
      envPuppeteer: !!process.env.PUPPETEER_EXECUTABLE_PATH,
      envSkipDownload: process.env.PUPPETEER_SKIP_DOWNLOAD,
    });
    return;
  }
  ...
  if (pn === '/api/qishui/logout') {
    saveQishuiCookie('');
    sendJSON(res, { provider: 'qishui', ok: true, loggedIn: false });
    return;
  }
  ...
  if (pn === '/api/login/qr/create') {
    try {
      ...
      if (provider !== 'netease') {
        const qrUrl = PLATFORM_QR_URLS[provider] || PLATFORM_QR_URLS.netease;
        const img = await QRCode.toDataURL(qrUrl, { width: 280, margin: 2, color: { dark: '#000000', light: '#ffffff' } });
        sendJSON(res, { img, url: qrUrl, provider });
        return;
      }
      const key = url.searchParams.get('key');
  ...
  if (pn === '/api/login/qr/check') {
    try {
      ...
      if (provider !== 'netease') {
        // 其他平台无法自动轮询，返回提示
        sendJSON(res, { code: 801, message: '请使用手机扫码登录后，点击下方导入 Cookie', provider });
        return;
      }
      const key = url.searchParams.get('key');
  ...
  if (pn === '/api/login/cookie') {
    try {
      const body = await readRequestBody(req);
      const raw = body.cookie || body.data || body.text || '';
      const provider = body.provider || 'netease';
      const normalized = normalizeCookieHeader(raw);
      const ut = _currentUserToken;

      // 🧑‍🤝‍🧑 多用户：优先保存到用户会话
      if (ut) {
        setUserCookie(provider, ut, normalized);
        sendJSON(res, { loggedIn: true, provider, saved: true, multiUser: true });
        return;
      }

      // 无 userToken 时回退到全局
      if (provider === 'kugou') { saveKugouCookie(normalized); sendJSON(res, { loggedIn: true, provider: 'kugou', saved: true }); return; }
      if (provider === 'kuwo') { saveKuwoCookie(normalized); sendJSON(res, { loggedIn: true, provider: 'kuwo', saved: true }); return; }
      if (provider === 'qishui') { saveQishuiCookie(normalized); sendJSON(res, { loggedIn: true, provider: 'qishui', saved: true }); return; }
      if (provider === 'qq') { saveQQCookie(normalized); sendJSON(res, { loggedIn: true, provider: 'qq', saved: true }); return; }
      const obj = parseCookieString(normalized);
      if (!obj.MUSIC_U) {
      ...
      if (!info.loggedIn && userCookie) {
        info = { loggedIn: true, pendingProfile: true, nickname: '网易云用户', avatar: '', vipType: 0, vipLevel: 'none', isVip: false, isSvip: false, vipLabel: '无VIP' };
      }
  ...
  if (pn === '/api/login/status') {
    const neteaseInfo = await getLoginInfo();
    const qqInfo = await getQQLoginInfo();
    const spotifyTokenInfo = loadSpotifyToken();
    let spotifyInfo = { loggedIn: !!(spotifyTokenInfo && spotifyTokenInfo.access_token), provider: 'spotify' };
    // 尝试获取Spotify用户信息
    if (spotifyInfo.loggedIn && spotifyTokenInfo) {
      try {
        const meRes = await fetch('https://api.spotify.com/v1/me', {
          headers: { 'Authorization': 'Bearer ' + spotifyTokenInfo.access_token }
        });
        if (meRes.ok) {
          ...
          spotifyInfo = {
            ...
            provider: 'spotify',
            userId: me.id || '',
            nickname: me.display_name || me.id || 'Spotify 用户',
    ...
    sendJSON(res, {
      netease: neteaseInfo,
      qq: qqInfo || { loggedIn: !!qqCookie, provider: 'qq' },
      kugou: await getKugouLoginInfo(),
      kuwo: await getKuwoLoginInfo(),
      qishui: await getQishuiLoginInfo(),
      spotify: spotifyInfo
    });
    return;
  }
  ...
  if (pn === '/api/spotify/oauth/callback') {
    try {
      ...
      if (error) { sendJSON(res, { loggedIn: false, error }); return; }
      if (!code) { sendJSON(res, { loggedIn: false, error: 'NO_CODE' }, 400); return; }
      const clientId = process.env.SPOTIFY_CLIENT_ID;
      const clientSecret = process.env.SPOTIFY_CLIENT_SECRET;
      const redirectUri = process.env.SPOTIFY_REDIRECT_URI || (req.headers.host ? 'http://' + req.headers.host + '/spotify-callback.html' : 'http://localhost:3000/spotify-callback.html');
      if (!clientId || !clientSecret) { sendJSON(res, { loggedIn: false, error: 'NO_SPOTIFY_CREDENTIALS' }, 400); return; }
      ...
      if (!tokenData.access_token) { sendJSON(res, { loggedIn: false, error: tokenData.error || 'TOKEN_FAILED' }, 400); return; }
      ...
      const me = await meRes.json();
      sendJSON(res, { loggedIn: true, provider: 'spotify', nickname: me.display_name || me.id, userId: me.id, avatar: me.images && me.images[0] && me.images[0].url });
    } catch (err) { sendJSON(res, { loggedIn: false, error: err.message }, 500); }
    return;
  }

  // ---------- Spotify OAuth 登录 URL ----------
  if (pn === '/api/spotify/oauth/url') {
    const clientId = process.env.SPOTIFY_CLIENT_ID;
    const redirectUri = process.env.SPOTIFY_REDIRECT_URI || (req.headers.host ? 'http://' + req.headers.host + '/spotify-callback.html' : 'http://localhost:3000/spotify-callback.html');
    if (!clientId) { sendJSON(res, { url: null, error: 'NO_SPOTIFY_CLIENT_ID' }); return; }
    ...
    const authUrl = 'https://accounts.spotify.com/authorize?response_type=code&client_id=' + encodeURIComponent(clientId) + '&scope=' + encodeURIComponent(scope) + '&redirect_uri=' + encodeURIComponent(redirectUri) + '&state=' + Date.now();
    sendJSON(res, { url: authUrl, redirectUri });
    return;
  }

  // ---------- Puppeteer 自动登录: 创建会话 ----------
  if (pn === '/api/login/puppeteer/create') {
    try {
      const provider = url.searchParams.get('provider') || 'netease';

      // ════════════════════════════════════════
      //  网易云：使用官方 API 生成二维码（无需浏览器）
      // ════════════════════════════════════════
      if (provider === 'netease') {
        console.log('[PuppeteerLogin] netease → 使用 API 二维码模式');
        const sessionId = crypto.randomUUID();

        // Step 1: 获取 QR key
        const keyResult = await login_qr_key({ timestamp: Date.now() });
        const qrKey = keyResult.body && keyResult.body.data && keyResult.body.data.unikey;
        if (!qrKey) {
          sendJSON(res, { error: '无法获取二维码密钥', status: 'error' }, 500);
          return;
        }

        // Step 2: 生成二维码图片
        const createResult = await login_qr_create({ key: qrKey, qrimg: true, timestamp: Date.now() });
        const qrData = createResult.body && createResult.body.data;
        const qrImg = qrData ? qrData.qrimg : null;
        const qrUrl = qrData ? qrData.qrurl : null;

        if (!qrImg) {
          sendJSON(res, { error: '无法生成二维码图片', status: 'error' }, 500);
          return;
        }
        ...
        activePuppeteerSessions.set(sessionId, apiSession);

        // 启动 API 轮询
        startNeteaseApiPolling(sessionId, qrKey);
      ...
      // ══════════════════════════════════════════════════════
      //  其他平台（QQ/酷狗/酷我/汽水）：Puppeteer 全自动登录
      //  启动浏览器 → 抓取二维码 → 自动检测扫码 → 自动提取 Cookie
      // ══════════════════════════════════════════════════════
      console.log(`[PuppeteerLogin] ${provider} → 启动 Puppeteer 全自动登录`);
      const result = await puppeteerLogin.createLoginSession(provider);
      sendJSON(res, result);
    } catch (err) {
      console.error('[PuppeteerLogin]', err);
  ...
  // ---------- Puppeteer 自动登录: 检查状态 ----------
  if (pn === '/api/login/puppeteer/check') {
    try {
      const sessionId = url.searchParams.get('sessionId');
      if (!sessionId) { sendJSON(res, { error: 'NO_SESSION_ID' }, 400); return; }

      // 🎯 先检查是否是 API 模式的网易云会话
      const apiSession = activePuppeteerSessions.get(sessionId);
      if (apiSession && apiSession.mode === 'api') {
        const result = {
          sessionId: apiSession.id,
          status: apiSession.status,
          provider: apiSession.provider,
          qrData: apiSession.qrData,
          qrUrl: apiSession.qrUrl,
          cookies: apiSession.status === 'logged_in' ? apiSession.cookies : null,
          error: apiSession.error,
          createdAt: apiSession.createdAt,
          mode: 'api',
        };

        // 登录成功时保存 cookie 到当前用户会话
        if (result.status === 'logged_in' && result.cookies) {
          const ut = _currentUserToken;
          if (ut) {
            setUserCookie('netease', ut, result.cookies);
            console.log(`[Puppeteer] ✅ 用户 ${ut.substr(0, 12)}... 已登录 netease (API模式)`);
          } else {
            saveCookie(result.cookies);
            console.log('[Puppeteer] ✅ 已登录 netease → 全局保存');
          }
        }
        sendJSON(res, result);
        return;
      ...
      // Puppeteer 浏览器模式（其他平台）
      const result = await puppeteerLogin.checkSession(sessionId);
      // 如果登录成功，保存cookie 到**当前用户的会话**（非全局）
      if (result.status === 'logged_in' && result.cookies) {
        const provider = result.provider;
        const ut = _currentUserToken; // 使用当前请求的用户 token
        if (ut) {
          // 🧑‍🤝‍🧑 多用户：保存到该用户的会话
          setUserCookie(provider, ut, result.cookies);
          console.log(`[Puppeteer] ✅ 用户 ${ut.substr(0, 12)}... 已登录 ${provider}`);
        } else {
          // 无 userToken 时回退到全局保存
          if (provider === 'netease') saveCookie(result.cookies);
          else if (provider === 'qq') saveQQCookie(result.cookies);
          else if (provider === 'kugou') saveKugouCookie(result.cookies);
          else if (provider === 'kuwo') saveKuwoCookie(result.cookies);
          else if (provider === 'qishui') saveQishuiCookie(result.cookies);
          else if (provider === 'spotify') {
            try { const tokenData = JSON.parse(result.cookies); saveSpotifyToken(tokenData); }
            catch (e) { saveSpotifyToken({ access_token: result.cookies, token_type: 'Bearer' }); }
          }
        }
      }
      sendJSON(res, result);
    } catch (err) {
      console.error('[PuppeteerCheck]', err);
      sendJSON(res, { error: err.message }, 500);
    }
    return;
  }

  // ---------- Puppeteer 自动登录: 获取二维码 ----------
  if (pn === '/api/login/puppeteer/qr') {
    try {
      const sessionId = url.searchParams.get('sessionId');
      if (!sessionId) { sendJSON(res, { error: 'NO_SESSION_ID' }, 400); return; }
      ...
      // Puppeteer 浏览器模式
      const result = await puppeteerLogin.getSessionQR(sessionId);
      sendJSON(res, result || { error: 'SESSION_NOT_FOUND' }, 404);
    } catch (err) {
      sendJSON(res, { error: err.message }, 500);
    }
    return;
  ...
  if (pn === '/api/user/playlists') {
    try {
      ...
      if (provider === 'netease') {
        ...
        sendJSON(res, { loggedIn: true, userId: info.userId, playlists: list, provider });
      } else if (provider === 'qq') {
        const qqPlaylists = await handleQQUserPlaylists();
        sendJSON(res, { ...qqPlaylists, provider });
      } else if (provider === 'spotify') {
        const token = loadSpotifyToken();
        if (!token || !token.access_token) { sendJSON(res, { loggedIn: false, playlists: [] }); return; }
        ...
        sendJSON(res, { loggedIn: true, userId: token.userId || '', playlists: list, provider });
      } else if (provider === 'kugou') {
        const playlists = await handleKugouUserPlaylists();
        sendJSON(res, { ...playlists, provider });
      } else if (provider === 'kuwo') {
        const playlists = await handleKuwoUserPlaylists();
        sendJSON(res, { ...playlists, provider });
      } else if (provider === 'qishui') {
        const playlists = await handleQishuiUserPlaylists();
        sendJSON(res, { ...playlists, provider });
      } else {
        sendJSON(res, { loggedIn: false, playlists: [], provider, message: '不支持的平台' });
      }
    } catch (err) {
  ...
  if (pn === '/api/song/like/check') {
    try {
      ...
      if (provider === 'netease') {
        const info = await getLoginInfo();
        if (!info.loggedIn || !info.userId) { sendJSON(res, { loggedIn: false, liked: {} }); return; }
        let likedIds = [];
        try {
        ...
        sendJSON(res, { loggedIn: true, ids, liked, provider });
      } else if (provider === 'qq') {
        const qqInfo = await getQQLoginInfo();
        if (!qqInfo.loggedIn) { sendJSON(res, { loggedIn: false, liked: {} }); return; }
        const uin = qqInfo.userId;
        // QQ 音乐红心/收藏检测：通过查询用户的收藏列表
        let likedIds = [];
        try {
          // 使用 QQ 音乐的 favorite 接口
          const favReq = await qqGetJSON('https://c.y.qq.com/fcg-bin/fcg_myQQ_fav', {
            g_tk: 5381,
            loginUin: uin,
            hostUin: 0,
            format: 'json',
            ...
            ein: 10000,
            type: 1, // 1=歌曲
          }, { headers: { Referer: 'https://y.qq.com/n/yqq/portal/profile.html' } });
          if (favReq && favReq.data) {
            const favSongs = (favReq.data.favList || favReq.data.list || []);
            likedIds = favSongs.map(s => String(s.songid || s.id || s.songId || s.mid || '')).filter(Boolean);
          }
        } catch (e) {
          console.warn('[QQLikeCheck]', e.message);
        }
        const set = new Set(likedIds);
        const liked = {};
        ids.forEach(id => { liked[id] = set.has(String(id)); });
        ...
        const token = loadSpotifyToken();
        if (!token || !token.access_token) { sendJSON(res, { loggedIn: false, liked: {} }); return; }
        // 查询用户是否收藏了这些歌曲
        const idsStr = ids.join(',');
        try {
          const r = await fetch('https://api.spotify.com/v1/me/tracks/contains?ids=' + encodeURIComponent(idsStr), {
            headers: { 'Authorization': 'Bearer ' + token.access_token }
          });
          const results = await r.json();
          const liked = {};
          if (Array.isArray(results)) {
            ids.forEach((id, i) => { liked[id] = !!results[i]; });
          } else {
            ids.forEach(id => { liked[id] = false; });
          }
          sendJSON(res, { loggedIn: true, ids, liked, provider });
        } catch (e) {
          sendJSON(res, { loggedIn: true, ids, liked: {}, provider, error: e.message });
        }
      } else {
        const cookieMap = { kugou: kugouCookie, kuwo: kuwoCookie, qishui: qishuiCookie };
        const hasCookie = !!(cookieMap[provider]);
        if (!hasCookie) { sendJSON(res, { loggedIn: false, liked: {} }); return; }
        // 非核心平台暂无红心检测API
        const liked = {};
        ids.forEach(id => { liked[id] = false; });
        sendJSON(res, { loggedIn: true, ids, liked, provider, message: provider + ' 红心检测功能开发中' });
      }
    } catch (err) {
  ...
  if (pn === '/api/song/like') {
    try {
      ...
      if (provider === 'netease') {
        const info = await getLoginInfo();
        if (!info.loggedIn) { sendJSON(res, { loggedIn: false, error: '未登录' }, 401); return; }
        ...
        sendJSON(res, { loggedIn: true, id, liked: nextLike, code, body: r.body || r, provider });
      } else if (provider === 'qq') {
        const qqInfo = await getQQLoginInfo();
        if (!qqInfo.loggedIn) { sendJSON(res, { loggedIn: false, error: '未登录' }, 401); return; }
        // 使用 QQ 音乐收藏/取消收藏接口
        try {
          const uin = qqInfo.userId;
          const actionUrl = nextLike
            ? 'https://c.y.qq.com/fcg-bin/fcg_add_song_to_fav'
            : 'https://c.y.qq.com/fcg-bin/fcg_del_fav';
          const result = await qqGetJSON(actionUrl, {
            g_tk: 5381,
            loginUin: uin,
            hostUin: 0,
            format: 'json',
            ...
            type: 1,
            sid: id,
          }, { headers: { Referer: 'https://y.qq.com/n/yqq/player' } });
          sendJSON(res, { loggedIn: true, id, liked: nextLike, provider, success: true, body: result });
        } catch (e) {
          sendJSON(res, { loggedIn: true, id, liked: nextLike, provider, message: e.message });
        }
      } else if (provider === 'spotify') {
        const token = loadSpotifyToken();
        if (!token || !token.access_token) { sendJSON(res, { loggedIn: false, error: '未登录' }, 401); return; }
        // Spotify save/remove track from library
        const method = nextLike ? 'PUT' : 'DELETE';
        const r = await fetch('https://api.spotify.com/v1/me/tracks?ids=' + encodeURIComponent(id), {
          method, headers: { 'Authorization': 'Bearer ' + token.access_token, 'Content-Type': 'application/json' }
        });
        if (r.ok || r.status === 200) {
          sendJSON(res, { loggedIn: true, id, liked: nextLike, provider, success: true });
        } else {
          const errData = await r.json().catch(() => ({}));
          sendJSON(res, { loggedIn: true, id, liked: nextLike, provider, error: errData.error && errData.error.message || '操作失败' }, 400);
        }
      } else {
        const cookieMap = { kugou: !!kugouCookie, kuwo: !!kuwoCookie, qishui: !!qishuiCookie };
        const hasCookie = !!(cookieMap[provider]);
        if (!hasCookie) { sendJSON(res, { loggedIn: false, error: '未登录' }, 401); return; }
        sendJSON(res, { loggedIn: true, id, liked: nextLike, provider, message: provider + ' 红心功能开发中，已记录操作意图' });
      }
    } catch (err) {
  ...
  if (pn === '/api/kugou/lyric') {
    try {
      ...
      sendJSON(res, { provider: 'kugou', lyric: '' }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kuwo/song/url') {
    try {
      ...
      const info = await handleKuwoSongUrl(rid);
      sendJSON(res, info);
    } catch (err) {
      console.error('[KuwoSongUrl]', err);
      sendJSON(res, { provider: 'kuwo', url: '', playable: false, error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/kuwo/lyric') {
    try {
      const rid = url.searchParams.get('rid') || url.searchParams.get('id') || '';
      const data = await handleKuwoLyric(rid);
      sendJSON(res, data);
    } catch (err) {
      console.error('[KuwoLyric]', err);
      sendJSON(res, { provider: 'kuwo', lyric: '' }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/qishui/song/url') {
    try {
      const id = url.searchParams.get('id') || '';
      const info = await handleQishuiSongUrl(id);
      sendJSON(res, info);
    } catch (err) {
  ...
  if (pn === '/api/qishui/lyric') {
    try {
      const id = url.searchParams.get('id') || '';
      const data = await handleQishuiLyric(id);
      sendJSON(res, data);
    } catch (err) {
      console.error('[QishuiLyric]', err);
      sendJSON(res, { provider: 'qishui', lyric: '' }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/spotify/song/url') {
    try {
      const id = url.searchParams.get('id') || '';
      const info = await handleSpotifySongUrl(id);
      sendJSON(res, info);
    } catch (err) {
  ...
  if (pn === '/api/spotify/lyric') {
    try {
      ...
      sendJSON(res, { provider: 'spotify', lyric: '' }, 500);
    }
    return;
  }

  // 🧑‍🤝‍🧑 多用户 Cookie：恢复全局变量（请求处理完毕后）
  if (_restoreGlobalCookie) {
    userCookie = _restoreGlobalCookie.userCookie;
    qqCookie = _restoreGlobalCookie.qqCookie;
    kugouCookie = _restoreGlobalCookie.kugouCookie;
    kuwoCookie = _restoreGlobalCookie.kuwoCookie;
    qishuiCookie = _restoreGlobalCookie.qishuiCookie;
    spotifyToken = _restoreGlobalCookie.spotifyToken;
  }
...
async function getKugouLoginInfo() {
  if (!kugouCookie) return { loggedIn: false, provider: 'kugou' };
  try {
    const res = await fetch('https://kugou.com/api/v1/user/info', {
      headers: { 'Cookie': kugouCookie, 'User-Agent': 'Mozilla/5.0' }
    });
    if (res.ok) {
      const data = await res.json();
      return { loggedIn: true, provider: 'kugou', nickname: data.nickname || '酷狗用户', avatar: data.avatar || '', vipType: data.vip_type || 0, vipLabel: data.vip_type ? 'VIP' : '免费' };
    }
  } catch(e) {}
  return { loggedIn: true, provider: 'kugou', nickname: '酷狗用户', avatar: '' };
}

async function getKuwoLoginInfo() {
  if (!kuwoCookie) return { loggedIn: false, provider: 'kuwo' };
  try {
    const res = await fetch('http://www.kuwo.cn/api/www/user/userInfo', {
      headers: { 'Cookie': kuwoCookie, 'User-Agent': 'Mozilla/5.0', 'Referer': 'http://www.kuwo.cn' }
    });
    if (res.ok) {
      const data = await res.json();
      return { loggedIn: true, provider: 'kuwo', nickname: data.data && data.data.nickname || '酷我用户', avatar: data.data && data.data.avatar || '', vipType: data.data && data.data.vipType || 0 };
    }
  } catch(e) {}
  return { loggedIn: true, provider: 'kuwo', nickname: '酷我用户', avatar: '' };
}

async function getQishuiLoginInfo() {
  if (!qishuiCookie) return { loggedIn: false, provider: 'qishui' };
  // 抖音没有公开API，返回基础信息
  return { loggedIn: true, provider: 'qishui', nickname: '抖音用户', avatar: '', vipType: 0, vipLabel: '未知' };
}

// 覆盖 /api/login/status 响应，加入用户信息
// （已在主路由中处理，此处确保函数存在）
